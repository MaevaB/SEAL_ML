...
BigPolyArray indicatorZero(BigPolyArray &encrypted, Evaluator &evaluator, BigPolyArray &one) {
  ...
  return evaluator.sub( encrypted, one);
}
...
BigPolyArray ind_function( Encryptor &encryptor, BalancedEncoder &encoder, Evaluator &evaluator, int **bin_decomp_label, vector <BigPolyArray> &user, BigPolyArray &one){
	...
	for (int i=0; i<MAX_BITS; ++i){
		if ((*bin_decomp_label)[i] == 0){
			cout << "*******2*****" << endl;
			encrypted_label_ind_func.push_back(indicatorZero( user.at(i), evaluator, one));
		}else{
			encrypted_label_ind_func.push_back( user.at(i));
		}
		cout << "*******3*****" << endl;
	}
	cout << "*******4*****" << endl;
	return evaluator.multiply_many(encrypted_label_ind_func);
}
...
int main(int argc, char **argv) {
  ...
  */
  ...
  for (int i=0; i<NB_USERS; i++) {
	...
	tmpRow.push_back(encryptor.encrypt(encoder.encode(tmpBitDecomp[k])));
      }
      // Finally, we add the encrypted value at the end
      tmpRow.push_back(encryptor.encrypt(encoder.encode(tmpRand)));
